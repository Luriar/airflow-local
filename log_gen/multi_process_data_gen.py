...
KINESIS_DATA_STREAM_NAME = 'de-ai-12-ap2-kdf-medallion-bronze-stream'
...
def gen_data(store_id):
  ...
  raw_log = {
      ...
      "data": {             # 상세 데이터 -> dict내에 dict 구성, 객체직렬화 추가
          "user_id": f"user_{random.randint(100, 999)}", # 사용자 id (사용자별로 여러번 구매)
          "item_id": selected_item["item_id"],           # 구매 제품
          "price": selected_item["price"],               # 단가
          "qty": qty,                                    # 수량
          "store_id": store_id                           # 매장 번호(오프/온라인 포함)
      },
      "ingested_at": current_utc_time # 로그 ㅡ발생 시간(event_time) 동일
  }
  return raw_log
  pass
...
def send_to_kinesis(log_entry):
  ...
  '''
  try:
    # PartitionKey -> ... -> 샤드(전용 차선)의 개수에 영향을 줌 -> 용량(가변(온디맨드), 고정(프로비저닝)) -> 운영 비용 및 성능 영향
    # 샤드에 대응되는 컬럼(키) 지정 -> log_entry['event_id'] -> 중복되지 않는 가지 수 등장
    # 몇 개의 샤드가 필요하지? -> log_entry['event_id'] 해싱처리 -> 수치화 -> 구간화 -> 샤드 배치
    # 1개의 샤드에 여러개의 log_entry['event_id']들이 배치가 됨 -> 분산 구조(골고루)
    # 아니라면 특정 샤드에 몰릴 수 있음 -> 부하가 발생돼서 성능이 저하될 수 있음
    # 적정 개수를 모르겠다 -> 온디맨드 -> 테스트 -> 적정 샤드 수 산출 -> 프로비저닝(서비스할 )
