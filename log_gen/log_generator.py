...
class LogGenerator:
  ...
  # self는 클래스 내부에서만 보이고, 외부에서는 없다고 판단(1번인자 self 아님)
  def get_interval_time(self, mode, interval):
    '''
      로그 발생 후 다음 로그 발생시까지 텀(sleep, 대기)하는 간격 계산
    '''
    if mode == 'fixed':
      return interval # 고정 주기로 로그 발생
    return random.uniform(0.1, interval*2) # 0.1 ~ 고정 주기 * 2배 : 무작위성

  # 추후. 중복데이터, 불량데이터 등등 발생 기능은 확장
  # 도메인별 로그 발생 함수 -> 상세 도메인/샘플 참조 유리1
  # 1. 금융 로그
  def finance(self):
    return {
        # 로그 발생 시간
        ...
        # 838aef74-da2f-44c3-8d86-308b2f1d2f4a : 36wkfl
