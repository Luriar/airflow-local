'''
- 목표
    - 데이터 생산(etl등 통해서) -> CSV -> S3 업로드(PUSH) 처리
    - 배치 작업(특정 시간대에 스케쥴링하여 일괄 처리) -> airflow 목표
'''

# 1. 모듈 가져오기
from datetime import datetime, timedelta
from airflow import DAG
from airflow.operators.python import PythonOperator
from airflow.operators.bash import BashOperator
import logging
from airflow.providers.amazon.aws.transfers.local_to_s3 import LocalFilesystemToS3Operator
from airflow.providers.amazon.aws.hooks.s3 import S3Hook
import pandas as pd
import random
import os

# 2-1. 버킷명 : {계정} - {root계정아이디}-{리전} <- 서비스명 누락 -> 차후 리소스명 네이밍 컨벤션 체크
BUCKET_NAME = "de-ai-12-827913617635-ap-northeast-2-an"

# 2-2. 업로드할 파일명
FILE_NAME = "sensor_data.csv"

# 버킷내에 특정 폴더 위치에 생성 -> KEY 지정
S3_KEY = f"income/{FILE_NAME}"

# 업로드할 로컬 경로 (컨테이너 -> 리눅스 기반)
LOCAL_PATH = f"/opt/airflow/dags/data/{FILE_NAME}"


def _create_dummy_data_csv(**kwargs):
    # 더미 센서 데이터를 csv로 생성
    os.makedirs("/opt/airflow/dags/data", exist_ok=True)

    data = [
        {
            "sensor_id": f"SENSOR_{i+1}",
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "temperature": round(random.uniform(20.0, 150.0), 2),
            "status": "on"
        }
        for i in range(10)
    ]

    df = pd.DataFrame(data)
    df.to_csv(LOCAL_PATH, index=False)

    logging.info(f"csv 생성 완료 -> {LOCAL_PATH}")
    return LOCAL_PATH


def _check_s3(**kwargs):
    # S3Hook을 통해서 실제 파일이 존재하는지 체크
    hook = S3Hook(aws_conn_id='aws_default')

    is_exist = hook.check_for_key(
        key=S3_KEY,
        bucket_name=BUCKET_NAME
    )

    if not is_exist:
        raise ValueError('업로드 실패')

    logging.info(f'업로드 확인 완료 -> bucket:{BUCKET_NAME}, key:{S3_KEY}')


# 3. DAG 정의
with DAG(
    dag_id="09_aws_s3_producer",
    description="센서 데이터를 csv 파일로 생성하여 S3에 업로드하는 producer DAG",
    default_args={
        'owner': 'de_2team_manager',
        'retries': 1,
        'retry_delay': timedelta(minutes=1)
    },
    schedule_interval=None,  # 스케쥴 x -> 트리거 작동으로 실행
    start_date=datetime(2026, 2, 25),
    catchup=False,
    tags=['aws', 's3', 'producer'],
) as dag:
    # 4. 오퍼레이터를 통한 TASK 정의
    task_create_dummy_data_csv = PythonOperator(
        task_id="task_create_dummy_data_csv",
        bash_command = f'echo "id,timestamp,value\n1,$(date),100\n2,$(date),500" > {LOCAL_PATH}'
    )

    task_upload_to_s3 = LocalFilesystemToS3Operator(
        task_id="upload_to_s3",
        filename=LOCAL_PATH,
        dest_key=S3_KEY,            # 버킷내 특정 위치
        dest_bucket=BUCKET_NAME,
        aws_conn_id='aws_default',
        replace=True                # 동일 파일이 있으면 덮는다 -> 그 순간 파일은 1개 (멱등성)
    )

    task_check_s3 = PythonOperator(
        task_id="check_s3",
        python_callable=_check_s3
    )

    # 5. 의존성, injection
    task_create_dummy_data_csv >> task_upload_to_s3 >> task_check_s3