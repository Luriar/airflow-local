...
BUCKET_NAME = "de-ai-12-827913617635-ap-northeast-2-an" # 글로벌하게 고유한 이름 사용!
# 2-2. 업로드할 파일명 준비
FILE_NAME = 'hello.txt'
...
def _check_s3(**kwargs):
    ...
    # 2. 훅을 이용하여 모든 키(객체명, 파일명 등등...) 조회
    keys = hook.list_keys(bucket_name=BUCKET_NAME)
    # 3. 키 체크
    if not keys:
        raise ValueError('업로드 실패') # 현재 파일만 판단 -> 추후 버킷 내 경로를 디테일하게 세분화
    # 4. 실제로 존재하면 로그 출력
    for key in keys:
