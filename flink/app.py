...
- 입력 티이블, 출력 테이블, 조회 및 전송
- 표준 SQL + AWS + Flink 특징점 추가됨 형태
- Flink를 이용하면 데이터를 배치|스트리밍 등 어떤 방식이든 분석에 적합한 데이터 형태로 가공할 수 있음
- 자바|스칼라|파이썬 + SQL 결합하여 처리 가능함
...
def main():
    ...
    '''
        로그 원문 1개
        {
            'event_time': '2026-04-16T11:22:47.562211',
            'ticker': 'NVIDIA',
            'price': 432.33,
            'volume': 40,
            'trade_id': 2028174
    ...
    t_env.execute_sql('''
        create table stock_output (
            ...
            price DOUBLE,
                      ...
                      "connector" = "kinesis",
                      "stream" = "de-ai-12-an2-kds-stock-output",
                      "aws.region" = "ap-northeast-2",
                      "scan.stream.initpos" = "LATEST",
                      "format" = "json"
                      )
